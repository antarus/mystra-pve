 ...
 * @project Mystra
 */
class RaceController extends \Zend\Mvc\Controller\AbstractActionController
{
    ...
    {
        if (!$this->_table) {
            $this->_table = $this->getServiceLocator()->get('\Commun\Table\RaceTable');
        }
        return $this->_table;
    }
    ...
    public function listAction()
    {
        ...
        $oViewModel->setTemplate('commun/race/list');
        return $oViewModel;
    }
    ...
    {
        $oTable = new \Commun\Grid\RaceGrid($this->getServiceLocator(),$this->getPluginManager());
    ...
    public function createAction()
    {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La race a été créé avec succès."), 'success');
                return $this->redirect()->toRoute('commun-race-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/race/create');
    ...
    public function updateAction()
    {
        ...
        try {
            ...
            if (!$oEntite) {
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("Identifiant de race inconnu."), 'error');
                return $this->redirect()->toRoute('commun-race-list');
            }
        } catch (Exception $ex) {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La race a été modifié avec succès."), 'success');
                return $this->redirect()->toRoute('commun-race-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/race/update');
    ...
    public function deleteAction()
    {
        ...
        $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La race a été supprimé avec succès."), 'success');
        return $this->redirect()->toRoute('commun-race-list');
    }
