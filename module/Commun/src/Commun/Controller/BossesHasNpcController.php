 ...
 * @project Mystra
 */
class BossesHasNpcController extends \Zend\Mvc\Controller\AbstractActionController
{
    ...
    {
        if (!$this->_table) {
            $this->_table = $this->getServiceLocator()->get('\Commun\Table\BossesHasNpcTable');
        }
        return $this->_table;
    }
    ...
    public function listAction()
    {
        ...
        $oViewModel->setTemplate('commun/bosses-has-npc/list');
        return $oViewModel;
    }
    ...
    {
        $oTable = new \Commun\Grid\BossesHasNpcGrid($this->getServiceLocator(),$this->getPluginManager());
    ...
    public function createAction()
    {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La bosses-has-npc a été créé avec succès."), 'success');
                return $this->redirect()->toRoute('commun-bosses-has-npc-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/bosses-has-npc/create');
    ...
    public function updateAction()
    {
        ...
        try {
            ...
            if (!$oEntite) {
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("Identifiant de bosses-has-npc inconnu."), 'error');
                return $this->redirect()->toRoute('commun-bosses-has-npc-list');
            }
        } catch (Exception $ex) {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La bosses-has-npc a été modifié avec succès."), 'success');
                return $this->redirect()->toRoute('commun-bosses-has-npc-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/bosses-has-npc/update');
    ...
    public function deleteAction()
    {
        ...
        $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La bosses-has-npc a été supprimé avec succès."), 'success');
        return $this->redirect()->toRoute('commun-bosses-has-npc-list');
    }
