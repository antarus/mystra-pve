 ...
 * @project Mystra
 */
class EvenementsRolesController extends \Zend\Mvc\Controller\AbstractActionController
{
    ...
    {
        if (!$this->_table) {
            $this->_table = $this->getServiceLocator()->get('\Commun\Table\EvenementsRolesTable');
        }
        return $this->_table;
    }
    ...
    public function listAction()
    {
        ...
        $oViewModel->setTemplate('commun/evenements-roles/list');
        return $oViewModel;
    }
    ...
    {
        $oTable = new \Commun\Grid\EvenementsRolesGrid($this->getServiceLocator(),$this->getPluginManager());
    ...
    {
        $oForm = new \Commun\Form\EvenementsRolesForm();//new \Commun\Form\EvenementsRolesForm($this->getServiceLocator());
        $oRequest = $this->getRequest();
        
        $oFiltre = new \Commun\Filter\EvenementsRolesFilter();
        $oForm->setInputFilter($oFiltre->getInputFilter());
        
        if ($oRequest->isPost()) {
            $oEntite = new \Commun\Model\EvenementsRoles();
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-roles a été créé avec succès."), 'success');
                return $this->redirect()->toRoute('commun-evenements-roles-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/evenements-roles/create');
    ...
    public function updateAction()
    {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-roles a été modifié avec succès."), 'success');
                return $this->redirect()->toRoute('commun-evenements-roles-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/evenements-roles/update');
    ...
    public function deleteAction()
    {
        ...
        $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-roles a été supprimé avec succès."), 'success');
        return $this->redirect()->toRoute('commun-evenements-roles-list');
    }
