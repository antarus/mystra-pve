 ...
 * @project Mystra
 */
class EvenementsTemplateRolesController extends \Zend\Mvc\Controller\AbstractActionController
{
    ...
    {
        if (!$this->_table) {
            $this->_table = $this->getServiceLocator()->get('\Commun\Table\EvenementsTemplateRolesTable');
        }
        return $this->_table;
    }
    ...
    public function listAction()
    {
        ...
        $oViewModel->setTemplate('commun/evenements-template-roles/list');
        return $oViewModel;
    }
    ...
    {
        $oTable = new \Commun\Grid\EvenementsTemplateRolesGrid($this->getServiceLocator(),$this->getPluginManager());
    ...
    public function createAction()
    {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-template-roles a été créé avec succès."), 'success');
                return $this->redirect()->toRoute('commun-evenements-template-roles-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/evenements-template-roles/create');
    ...
    public function updateAction()
    {
        ...
        try {
            ...
            if (!$oEntite) {
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("Identifiant de evenements-template-roles inconnu."), 'error');
                return $this->redirect()->toRoute('commun-evenements-template-roles-list');
            }
        } catch (Exception $ex) {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-template-roles a été modifié avec succès."), 'success');
                return $this->redirect()->toRoute('commun-evenements-template-roles-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/evenements-template-roles/update');
    ...
    public function deleteAction()
    {
        ...
        $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-template-roles a été supprimé avec succès."), 'success');
        return $this->redirect()->toRoute('commun-evenements-template-roles-list');
    }
