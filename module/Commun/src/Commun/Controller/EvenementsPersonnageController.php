...
class EvenementsPersonnageController extends \Zend\Mvc\Controller\AbstractActionController
{
    ...
    {
        if (!$this->_table) {
            $this->_table = $this->getServiceLocator()->get('\Commun\Table\EvenementsPersonnageTable');
        }
        return $this->_table;
    }
    ...
    public function listAction()
    {
        ...
        $oViewModel->setTemplate('commun/evenements-personnage/list');
        return $oViewModel;
    }
    ...
    {
        $oTable = new \Commun\Grid\EvenementsPersonnageGrid($this->getServiceLocator(),$this->getPluginManager());
    ...
    public function createAction()
    {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-personnage a été créé avec succès."), 'success');
                return $this->redirect()->toRoute('commun-evenements-personnage-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/evenements-personnage/create');
    ...
    public function updateAction()
    {
        ...
        if ($oRequest->isPost()) {
            ...
            if ($oForm->isValid()) {
                ...
                $this->flashMessenger()->addMessage($this->_getServTranslator()->translate("La evenements-personnage a été modifié avec succès."), 'success');
                return $this->redirect()->toRoute('commun-evenements-personnage-list');
            }
        }
        // Pour optimiser le rendu
        $oViewModel = new ViewModel();
        $oViewModel->setTemplate('commun/evenements-personnage/update');
